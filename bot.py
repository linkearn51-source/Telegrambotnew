...
async def buttons(client, query):
    ...
    if query.data == "upload":
        ...
        await query.message.reply("📤 Kirim file/video")

    elif query.data == "create":

        if user not in user_upload or len(user_upload[user]) == 0:
            return await query.answer("Upload dulu", True)

        code = str(uuid.uuid4())[:8]

        codes[code] = user_upload[user]
        ...
        total = len(user_upload.get(user, []))
...
async def save_file(client, message):
    ...
    user_upload[user].append(msg.id)

    total = len(user_upload[user])
