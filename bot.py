...
async def start(client,message):
    ...
    if len(args) > 1:
        ...
        if code in created_links:
            ...
            await send_page(client,message.chat.id,files,0)

            return

    # ========= NORMAL START HARUS JOIN =========
...
async def start_button(client,message):
...
async def send_page(client,chat_id,files,page):
...
async def page_handler(client,query):
    ...
    files=list(created_links.values())[-1]

    await send_page(client,query.message.chat.id,files,page)
...
async def broadcast_button(client,message):
